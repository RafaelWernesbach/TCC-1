...
\documentclass[
	...
	openany,			  % capítulos começam em página ímpar
	oneside,			  % impressão nos dois lados do papel
...
\titulo{Modelagem e Implementação de uma Solução de Automação do Fluxo Pedagógico no Sistema Educa EMES}

% Informe o(s) autor(es). Se houver mais de 1 autor, separar os autores com o
% comando " \and " (sem as aspas).
\autor{Rafael Christian Silva Wernesbach}
...
\palavraschave{Automação de processos educacionais, sistemas de informação educacional, modelagem de processos, BPMN, fluxo pedagógico.}
...
% PDF final gerado.
\keywords{Educational process automation, educational information systems, process modeling, BPMN, pedagogical workflow.}
...
\autorcutter{Silva Wernesbach, Rafael Christian}
...
% tiver dificuldade, peça auxílio à bibliotecária da UVV.
\assunto{%
1. Tecnologia da informação aplicada à educação.
2. Gestão de processos acadêmicos.
3. Automação de processos pedagógicos.
4. Transformação digital no setor público.
5. Sistemas integrados de gestão educacional.
I. Araújo Silva Filho, Abrantes.           % Sobrenome e nome: 2º autor OU orientador
% II. Sobrenome, Nome.          % Sobrenome e nome: orientador OU coorientador
II. \imprimirnomeuvv  .      % Sobrenome e nome do coorientador OU nome UVV
III. Título.                   % Nome UVV OU a palavra "Título"
