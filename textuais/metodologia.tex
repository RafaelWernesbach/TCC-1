...
A análise do sistema atual teve como objetivo compreender a estrutura técnica do Educa EMES, identificando os principais componentes tecnológicos, a organização da aplicação e o modelo de persistência utilizado para sustentar o fluxo pedagógico. Essa etapa foi necessária para contextualizar a proposta de automação dentro de uma solução já existente, em funcionamento e integrada a diferentes rotinas acadêmicas e administrativas da EMES.

O Educa EMES é uma aplicação web desenvolvida em PHP, utilizando o framework Laravel como base arquitetural. A escolha por Laravel permite a organização da aplicação segundo o padrão Model-View-Controller (MVC), separando responsabilidades entre regras de negócio, persistência de dados, controle de requisições e camada de apresentação. Além disso, o sistema utiliza recursos do ecossistema Laravel, como Eloquent ORM para mapeamento objeto-relacional, migrations e models para representação das entidades do domínio, além de mecanismos de autenticação, autorização, notificações, filas e comandos de console.

Na camada de interface administrativa, o sistema utiliza o Filament, framework construído sobre Laravel, Livewire e Blade, empregado para acelerar a construção de painéis administrativos, formulários, tabelas, filtros e ações operacionais. Essa escolha é relevante para o contexto da EMES porque grande parte das rotinas pedagógicas e administrativas depende de interfaces internas de cadastro, acompanhamento, validação e execução de procedimentos pelos servidores. Dessa forma, o Filament atua como camada de operação do sistema, enquanto Laravel concentra a estrutura de aplicação e as regras de negócio.

O sistema utiliza banco de dados relacional, com persistência estruturada em MySQL/MariaDB. O uso de um Sistema Gerenciador de Banco de Dados relacional é adequado ao domínio analisado, uma vez que o fluxo pedagógico depende de entidades fortemente relacionadas, como cursos, atividades, inscrições, alunos, instrutores, frequências, certificados, agendas, públicos-alvo, comunicações e avaliações. A integridade dessas relações é essencial para garantir consistência entre as etapas do processo acadêmico, especialmente nos pontos em que uma ação posterior depende de registros anteriores, como a geração de frequência a partir de uma inscrição confirmada ou a emissão de certificados a partir da aprovação do participante.

A análise do modelo de dados foi realizada a partir da estrutura do banco de dados e dos models da aplicação, considerando apenas as tabelas e relacionamentos relevantes ao fluxo pedagógico e administrativo abordado neste trabalho. Tabelas, campos e estruturas legadas, sem relação direta com o escopo da automação proposta, foram desconsiderados nesta etapa, a fim de preservar o foco da análise e evitar a inclusão de elementos que não contribuem para a compreensão do problema investigado.

No recorte analisado, a entidade central do sistema é a tabela de cursos, responsável por representar cursos, eventos e atividades educacionais vinculadas ao fluxo pedagógico. A partir dela se relacionam os principais registros operacionais do sistema, como inscrições de alunos, frequências, certificados, instrutores, arquivos, recursos, público-alvo, agendas e comunicações. Essa centralidade demonstra que o curso atua como unidade agregadora do processo acadêmico, concentrando tanto informações de planejamento quanto dados necessários à execução e ao encerramento das ações educacionais.

Além das entidades diretamente pedagógicas, o sistema também utiliza tabelas auxiliares para classificação e parametrização de informações. Essas estruturas armazenam dados como modalidade, tipo de evento, tipo de atividade, situação de inscrição, situação de frequência, tipo de público e demais classificações utilizadas ao longo do sistema. Esse modelo reduz a duplicação de tabelas específicas para cada categoria e permite maior flexibilidade na parametrização de valores utilizados pela aplicação. Contudo, também exige organização e governança sobre os grupos e códigos cadastrados, pois essas informações passam a influenciar diretamente regras de negócio e filtros operacionais.

A estrutura atual também contempla campos voltados à automação do fluxo pedagógico, armazenados na própria entidade de curso. Esses campos permitem identificar se determinado curso está habilitado para processamento automático, registrar configurações específicas de automação, armazenar o estado das etapas automatizadas e manter informações sobre o status geral do fluxo. Essa característica é especialmente relevante para a proposta deste trabalho, pois demonstra que a automação não será tratada como um sistema externo isolado, mas como uma evolução integrada ao modelo de dados já existente.

Dessa forma, a análise do sistema atual permite compreender a base técnica sobre a qual a solução proposta será desenvolvida. O estudo da stack tecnológica, do banco de dados relacional e dos principais relacionamentos entre entidades fornece subsídios para a definição dos requisitos, para a modelagem da automação e para a identificação dos pontos em que a solução poderá atuar sem comprometer a estrutura existente.
